...
**2.3. Архитектура класса `YandexSTTStreamer`:**
  ...
  * Класс должен управлять жизненным циклом gRPC-соединения и фоновых задач. Соединениями он управляет через класс `ConnectionManager`, который в фоне автоматически прогревает соединения и модель. Так мы получаем всегда открыето соединение на каждый звонок. `YandexSTTStreamer` сам создает экземпляр класса при инициализации. 
  * **Конструктор `__init__`:** Принимает `STTConfig` и объект учетных данных `credentials`. Внутри себя инициализирут `ConnectionManager` который создает прогретое соединение на основе класса по работе с пулом соединений из `stt_connection_pool.py`
  * **Основной метод `start_recognition(...)`:**
      ...
      * Берет соединение из `await ConnectionManager.acquire_connection()`. 
